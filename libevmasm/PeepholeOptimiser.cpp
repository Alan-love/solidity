...
struct SimplePeepholeOptimizerMethod
{
	static bool apply(OptimiserState& _state)
	{
		if (
			...
			ApplyRule<Method, WindowSize>::applyRule(_state.items.begin() + ptrdiff_t(_state.i), _state.out)
		)
		{
			_state.i += WindowSize;
			return true;
...
struct UnreachableCode
{
	static bool apply(OptimiserState& _state)
	{
		auto it = _state.items.begin() + ptrdiff_t(_state.i);
...
{
	return size_t(std::count(_items.begin(), _items.end(), Instruction::POP));
}
