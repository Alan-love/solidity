...
void CSECodeGenerator::generateClassElement(Id _c, bool _allowSequenced)
{
	...
	for (size_t i = 0; i < arguments.size(); ++i)
		assertThrow(m_stack[m_stackHeight - int(i)] == arguments[i], OptimizerException, "Expected arguments not present." );
	...
	for (size_t i = 0; i < arguments.size(); ++i)
	{
		m_classPositions[m_stack[m_stackHeight - int(i)]].erase(m_stackHeight - int(i));
		m_stack.erase(m_stackHeight - int(i));
	}
...
void CSECodeGenerator::appendDup(int _fromPosition, SourceLocation const& _location)
{
	...
	appendItem(AssemblyItem(dupInstruction(unsigned(instructionNum)), _location));
...
void CSECodeGenerator::appendOrRemoveSwap(int _fromPosition, SourceLocation const& _location)
{
	...
	assertThrow(instructionNum <= 16, StackTooDeepException, "Stack too deep, try removing local variables.");
	assertThrow(1 <= instructionNum, OptimizerException, "Invalid stack access.");
	appendItem(AssemblyItem(swapInstruction(unsigned(instructionNum)), _location));
