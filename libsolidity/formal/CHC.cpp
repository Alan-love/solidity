...
void CHC::analyze(SourceUnit const& _source)
{
	...
	for (auto const& [scope, target]: m_verificationTargets)
	{
		if (target.type == VerificationTarget::Type::Assert)
		{
			...
			{
				createErrorBlock();
				connectBlocks(target.value, error(), target.constraints && (target.errorId == size_t(assertion->id())));
		...
		else if (target.type == VerificationTarget::Type::PopEmptyArray)
		{
			...
			createErrorBlock();
			connectBlocks(target.value, error(), target.constraints && (target.errorId == size_t(scope->id())));
...
void CHC::visitAssert(FunctionCall const& _funCall)
{
	...
	connectBlocks(
		...
		currentPathConditions() && !m_context.expression(*args.front())->currentValue() && (m_error.currentValue() == size_t(_funCall.id()))
	);
...
void CHC::makeArrayPopVerificationTarget(FunctionCall const& _arrayPop)
{
	...
	connectBlocks(
		m_currentBlock,
		m_currentFunction->isConstructor() ? summary(*m_currentContract) : summary(*m_currentFunction),
		currentPathConditions() && symbArray->length() <= 0 && m_error.currentValue() == size_t(_arrayPop.id())
	);
