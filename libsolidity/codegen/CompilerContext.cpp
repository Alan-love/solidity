...
)
{
	...
	*this << lowLevelFunctionTag(_name, _inArgs, _outArgs, _generator);
	...
	adjustStackOffset(int(_outArgs) - 1 - int(_inArgs));
	*this << retTag.tag();
}
...
)
{
	...
	)
	{
		...
		auto stackDepth = size_t(_localVariables.end() - it);
		auto stackDiff = size_t(_assembly.stackHeight()) - startStackHeight + stackDepth;
		...
		if (_context == yul::IdentifierContext::RValue)
			_assembly.appendInstruction(dupInstruction(size_t(stackDiff)));
		else
		{
