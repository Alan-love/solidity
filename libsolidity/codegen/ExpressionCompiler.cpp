...
void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	m_context.adjustStackOffset(int(1 + CompilerUtils::sizeOnStack(paramTypes)));
...
bool ExpressionCompiler::visit(Conditional const& _condition)
{
	...
	m_context << trueTag;
	int offset = int(_condition.annotation().type->sizeOnStack());
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case FunctionType::Kind::Internal:
		{
			...
			m_context.adjustStackOffset(int(returnParametersSize - parameterSize) - 1);
			break;
		}
		...
		case FunctionType::Kind::SetGas:
		{
			...
			auto stackDepth = unsigned((function.gasSet() ? 1 : 0) + (function.valueSet() ? 1 : 0));
			if (stackDepth > 0)
		...
		{
			auto logNumber = unsigned(function.kind()) - unsigned(FunctionType::Kind::Log0);
		...
		case FunctionType::Kind::Require:
		{
			...
			else if (haveReasonString)
			{
				...
				m_context.adjustStackOffset(int(arguments.at(1)->annotation().type->sizeOnStack()));
			}
			else
				m_context.appendRevert();
			// the success branch
...
bool ExpressionCompiler::visit(FunctionCallOptions const& _functionCallOptions)
{
	...
	for (size_t i = 0; i < _functionCallOptions.options().size(); ++i)
	{
		...
		utils().moveIntoStack(size_t(insertPos), 1);
...
)
{
	...
	unsigned gasValueSize = unsigned((_functionType.gasSet() ? 1 : 0) + (_functionType.valueSet() ? 1 : 0));
	...
	auto remainsSize =
		unsigned(2) + // contract address, input_memory_end
