			...
			for (auto const& error: m_errorList)
				if (error.sourceName == name && error.locationStart >= 0 && error.locationEnd >= 0)
				{
					...
					for (int i = error.locationStart; i < error.locationEnd; i++)
						if (isWarning)
						{
							if (sourceFormatting[size_t(i)] == formatting::RESET)
								sourceFormatting[size_t(i)] = formatting::ORANGE_BACKGROUND_256;
						}
						else
							sourceFormatting[size_t(i)] = formatting::RED_BACKGROUND;
				}
