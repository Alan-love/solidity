...
void CodeTransform::operator()(VariableDeclaration const& _varDecl)
{
	...
	for (size_t varIndex = numVariables - 1; varIndex < numeric_limits<size_t>::max(); --varIndex)
	{
		YulString varName = _varDecl.variables[varIndex].name;
		auto& var = std::get<Scope::Variable>(m_scope->identifiers.at(varName));
		m_context->variableStackHeights[&var] = heightAtStart + int(varIndex);
		if (!m_allowStackOpt)
			continue;
...
void CodeTransform::operator()(FunctionCall const& _call)
{
	...
	else
	{
		...
		m_assembly.setSourceLocation(_call.location);
		if (m_evm15)
			m_assembly.appendJumpsub(functionEntryID(_call.functionName.name, *function), int(function->arguments.size()), int(function->returns.size()));
		else
		{
			m_assembly.appendJumpTo(functionEntryID(_call.functionName.name, *function), int(function->returns.size() - function->arguments.size()) - 1);
...
void CodeTransform::operator()(Identifier const& _identifier)
{
	...
	if (m_scope->lookup(_identifier.name, GenericVisitor{
		[&](Scope::Variable& _var)
		{
			...
			if (auto heightDiff = size_t(variableHeightDiff(_var, _identifier.name, false)))
...
void CodeTransform::operator()(FunctionDefinition const& _function)
{
	...
	if (m_evm15)
		m_assembly.appendBeginsub(functionEntryID(_function.name, function), int(_function.parameters.size()));
	else
...

	{
		...
		{
			StackTooDeepError error(_function.name, YulString{}, int(stackLayout.size()) - 17);
			...
			stackError(std::move(error), m_assembly.stackHeight() - int(_function.parameters.size()));
		}
		else
		{
			...
			for (size_t i = 0; size_t(i) < stackLayout.size(); ++i)
				yulAssert(i == size_t(stackLayout[i]), "Error reshuffling stack.");
		}
	}
	if (m_evm15)
		m_assembly.appendReturnsub(int(_function.returnVariables.size()), stackHeightBefore);
	else
		m_assembly.appendJump(stackHeightBefore - int(_function.returnVariables.size()));
...
void CodeTransform::generateAssignment(Identifier const& _variableName)
{
	...
	if (auto var = m_scope->lookup(_variableName.name))
	{
		...
		if (auto heightDiff = size_t(variableHeightDiff(_var, _variableName.name, true)))
